...
tableHeader='<tr><th>Start<th>End'//https://www.w3.org/TR/html401/struct/tables.html
function returnTableRowString(post,milliseconds) {//e.g. use as returnTableRowString(post,Date.now())
	return `<tr><td>${post.start}<td>${post.end}</table>`}
function returnTableData(post){//,milliseconds){
    console.log('returnTableData post: ',post)
    if (post.start){
        return `<tr><td>${post.start}</table>`}
    if (post.end){
        return `<td>${post.end}</table>`
    }
}
...
var storage_directory='./work_hours_logs_by_day/'
let port=1028
...
function createServerListener(request,response) {//request is a readable stream (http.IncomingMessage), response is a writable stream (http.ServerResponse).
	...
	if (request.method=='GET') {
	    ...
	    else {GETDayLogs(request,response,html_header_string,html_footer_string)}
	}
...
function GETDayLogs(request,response,html_header_string,html_footer_string){//GET, i.e. page load, serve the form to fill, and any already posted data today
		...
		fs.readdir(storage_directory,(error,files)=>{
		                    console.log(files)
	                            ...
	                            console.log(files)
				    if (files.length>0){files.reverse()//Or use Array.pop() and Array.length in createReadFileStreamRespondRecurse function definition
                                               
					                createReadFileStreamRespondRecurseEnd(files,storage_directory,response,html_footer_string,checked_today,todaysDate+'.html')}
                                    else {//add a line to appendForm
                                        response.end(html_footer_string)}
				})
	...
	let fileRS=fs.createReadStream(storage_directory+filenames_array[0])
		...
		fileRS.on('open',()=>{
		    if (!checked_today) {if (todaysFile==filenames_array[0]){console.log('already file for today, don\'t rewrite date header')
                ...
                let start_or_end=''
		fileRS.on('data',(data)=>{
                    if (todaysFile=filenames_array[0]) {
                        table_string=table_string+data                     
                        /*why didn't these methods work for checking if table was completed loaded?
                        console.log('table_string.length: ',table_string.length)
                        console.log('table_string: ',table_string)
                        console.log('table_string.endsWith(\'</table>\'): ',table_string.endsWith('</table>'))
                        console.log('table_string.endsWith(\' \'): ',table_string.endsWith(' '))
                        console.log('table_string[table_string.length-1]: ',table_string[table_string.length-1])
                        console.log('table_string[table_string.length-2]: ',table_string[table_string.length-2])
                        console.log('table_string.indexOf(\'</table>\'): ',table_string.indexOf('<\table>'))
                        console.log('split: ',table_string.split(table_close_tag)[0])
                        console.log('split1: ',table_string.split(table_close_tag)[1])
                        console.log('split2: ',table_string.split(table_close_tag).length)*/
                        //if (table_string.endsWith('</table>')) {
                        if (table_string.split(table_close_tag).length==2){
                            let table_minus_close_tag=table_string.split(table_close_tag)[0]
                            const table_array=table_minus_close_tag.split('<tr>')
                            const last_row=table_array[table_array.length-1]
                            if (last_row.split('<td>').length==3) {start_or_end='start'}
                            else {start_or_end='end'}
                            console.log('last_row: '+last_row)
                            console.log('start_or_end: '+start_or_end)
                            console.log('last_row.split(\'<td>\').length',last_row.split('<td>').length)
                            response.write(table_minus_close_tag)
                        }
                    }
		...
		fileRS.on('end',()=>{
                    if (todaysFile=filenames_array[0]) {
                        switch (start_or_end) {
                        case 'start':
                            response.write('<tr><td><form method=\'post\'><input type=\'time\' name=\'start\'><input type=\'submit\' value=\'Post\'></form></table>')
                            break
                        case 'end':
                            response.write('<td><form method=\'post\'><input type=\'time\' name=\'end\'><input type=\'submit\' value=\'Post\'></form></table>')                                               
                            break}                      
...
function POSTToTodaysLog(request,response,tableOpenTag,tableHeader,returnTableRowString){//If the request is a POST, append new info to today's .html file.
		...
		request.on('end', () => {//At POST end, append to today's file (create if doesn't exist), then redirect to GET
		    ...
		    todaysFileRS.on('error',(error)=>{console.log('todaysFileRS Error: '+error)//If file does not exist yet,
				...
				todaysFileWS.on('ready',()=>{
					...
					//todaysFileWS.write(returnTableRowString(post,Date.now()))//and the new row.
                                    todaysFileWS.write(returnTableData(post))//,Date.now()))//and the new row.
					todaysFileWS.end()
				})
		    ...
		    todaysFileRS.on('readable',function () {//console.log('readable!')//https://nodejs.org/api/stream.html#stream_event_readable
                        //Need to determine if file is at 
                        while (data=this.read()){
                            s+=data
                            table_minus_close_tag2=s.split(table_close_tag)[0]

                        }
                        console.log('s: '+s)
			todaysFileWS=fs.createWriteStream(todaysFile,{flags:'r+'})
			todaysFileWS.write(table_minus_close_tag2+returnTableData(post))//RowString(post,Date.now()))}//append the new row
			response.writeHead(303,{"Location":"http://"+request.headers['host']})//Redirect to GET request
			return response.end()
				...
				/*fs.stat(todaysFile,(error,stats)=>{//Find size of today's file and so which byte to write new row over </table> tag.
				    ...
				    if (table_close_index>0) {todaysFileWS.write(returnTableData(post))}//RowString(post,Date.now()))}//append the new row
						response.writeHead(303,{"Location":"http://"+request.headers['host']})//Redirect to GET request
						return response.end()
				})*/
			})			
